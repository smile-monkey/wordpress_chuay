...
if ( class_exists( 'Easy_Digital_Downloads' ) ) :
	...
	// Change category labels
	if ( ! function_exists( 'edd_cat_labels_diff' ) ) {
		function edd_cat_labels_diff( $labels ) {
			$singular  = 'Difficulty';
			$plural    = 'Difficulties';
			$labels = get_labels($singular, $plural);
			return $labels;
	...
	add_filter( 'edd_download_category_labels_diff', 'edd_cat_labels_diff' );

	// change the category slug
	if ( ! function_exists( 'edd_cat_args_diff' ) ) {
		function edd_cat_args_diff( $args ){
			$args['rewrite'] = array( 'slug' => 'difficulty', 'with_front' => false, 'hierarchical' => true);
			return $args;
		}
	}
	add_filter( 'edd_download_category_args_diff', 'edd_cat_args_diff');
