...
/*		var pppp = Math.floor(setting.currentTime);
	  ...
	  .bind($.jPlayer.event.timeupdate, function(event){
		...
		var procLine = jQuery("#IDs"+parseInt(pppp)).html();
...
$('ul.dropdown-menu li').each(function (i) {
	  ...
	  jQuery(".my-new-list").html( "" );
		...
		jQuery.each( data, function( key, val ) {
		 ...
		 {
		  var xbr = '<br>';
		  //var key4 = '<a class="lyrics_text_color2">' + data[key][4] + '</a>';  //// 15 may 2017  svnlabs
		  var key4 = data[key][4];
		 }
		 
		 items.push( '<span class="lyrics_text_color" id="IDs'+ data[key][1] +'" name="IDs'+ data[key][1] +'"  data-start="'+ data[key][1] +'" data-stop="400">'+ data[key][0] + xbr + key4 +'</span>' );
		...
		var lyrics_text = items.join( "<br>" );
		

		jQuery(".my-new-list").html( lyrics_text );
