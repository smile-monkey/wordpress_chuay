...
/* svnlabs 6 sep 2017 Music difficulty */

function edd_setup_difficulty_taxonomies() {

	$slug     = defined( 'EDD_SLUG' ) ? EDD_SLUG : 'difficulty';
	...
	$category_args = apply_filters( 'edd_download_category_args_diff', array(
			'hierarchical' => true,
			'labels'       => apply_filters('edd_download_category_labels_diff', $category_labels),
			'show_ui'      => true,
			'query_var'    => 'difficulty_category',
			'rewrite'      => array('slug' => $slug . '/category', 'with_front' => false, 'hierarchical' => true ),
			'capabilities' => array( 'manage_terms' => 'manage_product_terms','edit_terms' => 'edit_product_terms','assign_terms' => 'assign_product_terms','delete_terms' => 'delete_product_terms' )
		)
	);
	register_taxonomy( 'difficulty_category', array('download'), $category_args );
	register_taxonomy_for_object_type( 'difficulty_category', 'download' );
	...
	$tag_args = apply_filters( 'edd_download_tag_args_diff', array(
			'hierarchical' => false,
			'labels'       => apply_filters( 'edd_download_tag_labels_diff', $tag_labels ),
			'show_ui'      => true,
			'query_var'    => 'difficulty_tag',
			'rewrite'      => array( 'slug' => $slug . '/tag', 'with_front' => false, 'hierarchical' => true  ),
			'capabilities' => array( 'manage_terms' => 'manage_product_terms','edit_terms' => 'edit_product_terms','assign_terms' => 'assign_product_terms','delete_terms' => 'delete_product_terms' )
		)
	);
	register_taxonomy( 'difficulty_tag', array( 'download' ), $tag_args );
	register_taxonomy_for_object_type( 'difficulty_tag', 'download' );
}
add_action( 'init', 'edd_setup_difficulty_taxonomies', 0 );
...
function edd_get_taxonomy_labels( $taxonomy = 'download_category' ) {
	$allowed_taxonomies = apply_filters( 'edd_allowed_download_taxonomies', array( 'download_category', 'download_tag', 'difficulty_category', 'difficulty_tag' ) );
