...
function edd_add_download_meta_box() {
	...
	foreach ( $post_types as $post_type ) {
		...
		/** Product Lyrics Sync */
		add_meta_box( 'edd_product_lyrics_sync', sprintf( __( '%1$s Lyrics Sync', 'easy-digital-downloads' ), edd_get_label_singular(), edd_get_label_plural() ), 'edd_render_product_lyrics_sync_meta_box', $post_type, 'normal', 'default' );
...
function edd_download_meta_box_save( $post_id, $post ) {
    ...
    //print_r($_REQUEST); // deletejson
	
	if(isset($_POST['deletejson']) && $_POST['deletejson']=="1") {
	
	//$mp3_lyrics = EDD_PLUGIN_DIR."assets/lyrics/".$post_id.".json";
	$mp3_lyrics = ABSPATH."lyrics/".$post_id.".json";
	
		
	file_put_contents($mp3_lyrics, "");
...
function edd_render_product_lyrics_sync_meta_box() {
	global $post;

	do_action( 'edd_product_lyrics_sync_meta_box_fields', $post->ID );
}
...
function inputtitle_submit_scripts() {
	wp_enqueue_script( 'inputtitle_submit', EDD_PLUGIN_URL . 'assets/js/lyrics-core.js', array( 'jquery' ) );
	wp_localize_script( 'inputtitle_submit', 'LY_Ajax', array(
...
/* svnlabs */
function myajax_inputtitleSubmit_func($post_id) {  /// update lyrics by AJAX
	// check nonce
	...
	//$mp3_lyrics = EDD_PLUGIN_DIR."assets/lyrics/".$_POST['post_ID'].".json";
	$mp3_lyrics = ABSPATH."lyrics/".$_POST['post_ID'].".json";
	...
	$passage[$_POST['ID']] = array($passage[$_POST['ID']][0], $_POST['Time'], $passage[$_POST['ID']][2], $passage[$_POST['ID']][3], $passage[$_POST['ID']][4]);
...
/* svnlabs */
function format_sec($t,$f=':') // t = seconds, f = separator 
{
...
/* svnlabs */
function edd_render_product_lyrics_sync_field( $post_id ) {
	//$product_notes = edd_get_product_notes( $post_id );
	
	//do_action( 'wp_enqueue_scripts', $post->ID );
	
	$meta = get_post_meta( $post_id );   //echo "<pre>"; print_r($meta);
	
	//$mp3_lyrics = EDD_PLUGIN_DIR."assets/lyrics/".$post_id.".json";
	//$mp3_lyrics2 = EDD_PLUGIN_DIR."assets/lyrics/".$post_id."_1.json";
	
	$mp3_lyrics = ABSPATH."lyrics/".$post_id.".json";
	$mp3_lyrics2 = ABSPATH."lyrics/".$post_id."_1.json";
	
		
	
	$lyrics1 = explode("\n", $meta['lyrics1'][0]);    /// create JSON first time
	$lyrics2 = explode("\n", $meta['lyrics2'][0]);
	
	//print_r($meta);
	
	//echo file_get_contents( get_attached_file($meta['lyrics1file'][0]) );
...
/*	if(isset($_REQUEST['delete']) && $_REQUEST['delete']=="1")
	{
	   ...
	   //file_put_contents($mp3_lyrics, "");
	   //die("HERE...");
	   $uu = admin_url()."post.php?post=".$post_id."&action=edit";
	   ?>
	   <script>window.location="<?php echo $uu; ?>";</script>
       <?php
	...
	//$lyricsz = file_get_contents($mp3_lyrics);
	
	//if(file_exists($mp3_lyrics) && $lyricsz!="")   //// check if json lyrics file already 
	
	//if(isset($_POST['deletejson']) && $_POST['deletejson']=="1") 
	
	$lyricsz = json_decode( file_get_contents($mp3_lyrics), true );  //// check if JSON is valid & full
	
	//print_r($lyricsz);
	
	if(file_exists($mp3_lyrics) && $lyricsz['ID0'][0]!="")
	{
	
	//$lyricsJ = json_decode($lyricsz, true);
	
	
	//echo "<pre>";  
	//echo count($lyricsJ); print_r($lyricsJ);
	//echo count($lyrics1); print_r($lyrics1); die();
	...
	$srtfile="no";  // svnlabs-v2 Remove “Insert *.srt”
	
	//if($meta['srtfile'][0]=="no")
	...
	foreach($lyrics1 as $m)   /// create JSON first time
	{
	...
	if( isset($lyrics2[$indx]) && $lyrics2[$indx]!="" && $lyrics2[$indx]!="null")
	 $LY2 = $lyrics2[$indx];
	else
	  $LY2 = "";
	
	//$lyrics["ID".$i] = array($m, $i, $dur, $j, $LY2);   //// $lyrics2[$indx]  to insert dual lyrics in JSON
	
	$lyrics["ID".$i] = array($m, 1, 1, rand(0,20), $LY2);  /// svnlabs 15 may 2017
	
	//$lyrics["ID".$i] = array($m, 1, "1", rand(0,20), $LY2);   //// $lyrics2[$indx]  to insert dual lyrics in JSON
	...
	file_put_contents($mp3_lyrics, $echo);  
	...
	$playlist2 = array();
	$subs2    = array();
	$state2   = SRT_STATE_SUBNUMBER;
	$subNum2  = 0;
	$subText2 = '';
	$subTime2 = '';
	
	
	 	foreach($lyrics2 as $line2) {
		switch($state2) {
			case SRT_STATE_SUBNUMBER:
				$subNum2 = trim($line2);
				$state2  = SRT_STATE_TIME;
				break;
	
			case SRT_STATE_TIME:
				$subTime2 = trim($line2);
				$state2   = SRT_STATE_TEXT;
				break;
			...
			case SRT_STATE_TEXT:
				if (trim($line2) == '') {
					$sub2 = new stdClass;
					$sub2->number = $subNum2;
					list($sub2->startTime, $sub2->stopTime) = explode(' --> ', $subTime2);
					$sub2->text   = $subText2;
					$subText2     = '';
					$state2       = SRT_STATE_SUBNUMBER;
	...
	if( isset($subs2[$i]->text) && $subs2[$i]->text!="" && $subs2[$i]->text!="null")
	  $LY2 = $subs2[$i]->text;
	else
	  $LY2 = "";  
	
	//$playlist['ID'.$i] = array($subs[$i]->text."\r", format_sec($subs[$i]->startTime), format_sec($subs[$i]->startTime), format_sec($subs[$i]->stopTime), $LY2."\r");
	
	$playlist['ID'.$i] = array($subs[$i]->text."\r", format_sec($subs[$i]->startTime), "1", rand(0,20), $LY2."\r");
...
<link rel="stylesheet" href="<?php echo EDD_PLUGIN_URL . 'assets/css/'; ?>lyrics-style.css">


<?php /*?><h1><a href="<?php echo admin_url(); ?>post.php?post=<?php echo $post_id; ?>&action=edit&delete=1" style="color:#FF0000;">Delete Old Sync Lyrics</a></h1> <?php */?>

<h1  style="color:#FF0000;"><input type="checkbox" name="deletejson" value="1"  />Delete Old Sync Lyrics</h1>




<?php /*?><p class="loading">
    <em><img src="<?php echo EDD_PLUGIN_URL . 'assets/images/'; ?>loader.gif" alt="Initializing audio"> Loading audio...</em>
</p><?php */?>
...
function updateTrackTime1(track){
  ...
  var procLine = jQuery(".ID"+parseInt(currTime)).text();
  
  //if(jQuery(".ID"+parseInt(currTime)))
  
  if( procLine!=null && procLine.length > 3 )
  {
   //jQuery("#passage-text").find("span").removeClass('speaking');
   
   console.log(prevSp);
   
   if(prevSp!="")
     jQuery(".ID"+prevSp).removeClass('speaking');
   else
     jQuery("#passage-text").find("span").removeClass('speaking'); 	  
   
   jQuery(".ID"+parseInt(currTime)).addClass('speaking');
   
   prevSp = parseInt(currTime);
...
function makeBorder(obj)
{
  ...
  jQuery("#passage-text").find("span").removeClass('clicked');
...
.clicked{
  ...
  background-color:#FFFFCC;
	...
	<p class="passage-audio" <?php /*?>hidden<?php */?>>  <?php /*?>svnlabs   30 jun <?php */?>
    <audio id="passage-audio" class="passage" controls style="width:100%"  ontimeupdate="updateTrackTime(this);">
        <source src="<?php echo wp_get_attachment_url( $meta['preview'][0] ); ?>" type="audio/mp3">
        <em class="error"><strong>Error:</strong> Your browser doesn't appear to support HTML5 Audio.</em>
    </audio>
    </p>
    
     <p class="playback-rate" style="display:none;" title="Note that increaseing the reading rate will decrease accuracy of word highlights">
            ...
            <p class="autofocus-current-word" style="display:none;">
...
<div id="passage-text" class="passage">
  ...
  <strong>Lyrics 1</strong> 
  <p>
  
  <?php /*?>svnlabs 30 jun 2017<?php */?>
  
  <?php foreach($passage as $k=>$p){ ?>
  <span onclick="makeBorder(this);" style="cursor:pointer;" id="<?php echo $k; ?>" data-dur="<?php echo $p[2]; ?>" class="ID<?php echo $p[1]; ?>" data-begin="<?php echo $p[1]; ?>" data-stop="<?php echo $p[3]; ?>"><?php echo $p[0]; ?></span><br />
  <?php } ?>
...
<?php /*?>    <strong>Lyrics 1</strong><p><?php echo nl2br($meta['lyrics1'][0]); ?></p><br /><?php */?>

<hr />

    <strong>Lyrics 2</strong><p><?php echo nl2br($meta['lyrics2'][0]); ?></p>    
    
	<p><?php _e( 'Click on Audio Play Button then click Lyrics Text while you listen same TEXT.', 'easy-digital-downloads' ); ?></p>
    
    <?php /*?> svnlabs 30 jun 2017 <?php */?>
<?php /*?>	<script src="<?php echo EDD_PLUGIN_URL . 'assets/js/'; ?>lyrics-read-along.js"></script>
    <script src="<?php echo EDD_PLUGIN_URL . 'assets/js/'; ?>lyrics-main.js"></script><?php */?>
...
add_action( 'edd_product_lyrics_sync_meta_box_fields', 'edd_render_product_lyrics_sync_field' );
